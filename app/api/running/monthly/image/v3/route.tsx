      ...
      >
        ...
        >
          {topoLines.map((line, i) => (
            <path key={i} d={line} fill="none" stroke="#3b82f6" strokeWidth="1" />
          ))}
        </svg>
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
            <span style={{ fontSize: 24, color: '#fafafa', fontWeight: 500 }}>{monthName}</span>
            <span style={{ fontSize: 14, color: '#525252' }}>when1life</span>
          </div>
          ...
          >
            <span style={{ fontSize: 72, color: '#fafafa', fontWeight: 600, lineHeight: 1 }}>
              {current.totalDistance}
            </span>
            <span style={{ fontSize: 24, color: '#737373', marginTop: 4 }}>kilometers</span>
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              ...
              >
                <span style={{ fontSize: 36, color: '#fafafa', fontWeight: 500 }}>{current.runCount}</span>
                <span style={{ fontSize: 12, color: '#737373', marginTop: 4 }}>runs</span>
                <span
                  style={{
                    fontSize: 12,
              ...
              >
                <span style={{ fontSize: 36, color: '#fafafa', fontWeight: 500 }}>{current.avgPace}</span>
                <span style={{ fontSize: 12, color: '#737373', marginTop: 4 }}>avg pace</span>
                <span
                  style={{
                    fontSize: 12,
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              ...
              >
                <span style={{ fontSize: 36, color: '#fafafa', fontWeight: 500 }}>{current.avgHr || '−'}</span>
                <span style={{ fontSize: 12, color: '#737373', marginTop: 4 }}>avg hr</span>
                <span
                  style={{
                    fontSize: 12,
                    color:
              ...
              >
                <span style={{ fontSize: 36, color: '#fafafa', fontWeight: 500 }}>
                  {Math.floor(current.totalDuration / 60)}:{(current.totalDuration % 60).toString().padStart(2, '0')}
                </span>
                <span style={{ fontSize: 12, color: '#737373', marginTop: 4 }}>total time</span>
                <span
                  style={{
                    fontSize: 12,
                    color: changes.durationChange > 0 ? '#22c55e' : changes.durationChange < 0 ? '#ef4444' : '#737373',
                    marginTop: 8,
                  }}
                >
                  {formatChange(changes.durationChange, '%')}
                </span>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <span style={{ fontSize: 14, color: '#d4d4d4', lineHeight: 1.4 }}>{summary}</span>
          </div>
        </div>
        ...
        >
          ...
          >
            <span style={{ fontSize: 72, color: '#fafafa', fontWeight: 600 }}>{current.runCount}</span>
            <span style={{ fontSize: 20, color: '#737373', marginTop: 8 }}>runs</span>
            <span style={{ fontSize: 14, color: '#525252', marginTop: 4 }}>this month</span>
          </div>
          ...
          >
            <span style={{ fontSize: 12, color: '#737373', marginBottom: 16 }}>Daily Distance</span>
            ...
            {dailyDistances.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <svg width={chartWidth} height={chartHeight} style={{ display: 'flex' }}>
                  ...
                  {chartPoints && (
                    <polygon
                      points={`10,${chartHeight - 10} ${chartPoints} ${chartWidth - 10},${chartHeight - 10}`}
                      fill="#22c55e"
                      opacity="0.15"
                    />
                  )}
                  ...
                  {/* 라인 차트 */}
                  {chartPoints && (
                    <polyline points={chartPoints} fill="none" stroke="#22c55e" strokeWidth="2" strokeLinejoin="round" />
                  )}

                  {/* 데이터 포인트 */}
                  {chartCircles.map((circle, i) => (
                    <circle key={i} cx={circle.x} cy={circle.y} r="4" fill="#22c55e" />
                  ))}
                </svg>

                {/* X축 레이블 */}
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
                  <span style={{ fontSize: 10, color: '#525252' }}>1</span>
                  <span style={{ fontSize: 10, color: '#525252' }}>10</span>
                  <span style={{ fontSize: 10, color: '#525252' }}>20</span>
                  <span style={{ fontSize: 10, color: '#525252' }}>{daysInMonth}</span>
                </div>
              </div>
            ) : (
          ...
          <div style={{ display: 'flex', marginTop: 'auto', paddingTop: 20 }}>
            <span style={{ fontSize: 12, color: '#404040' }}>Running Coach · when1.life</span>
          </div>
        </div>
      </div>
    ),
...
function generateTopoLines(): string[] {
  ...
  // 동심원 형태의 등고선 (불규칙하게) - 우측에 중심
  for (let i = 1; i <= 15; i++) {
    const baseRadius = i * 60;
    const points: string[] = [];

    for (let angle = 0; angle <= 360; angle += 5) {
      const rad = (angle * Math.PI) / 180;
      const variation = Math.sin(angle * 0.08) * 25 + Math.cos(angle * 0.12) * 15;
      const radius = baseRadius + variation + Math.sin(i * 0.4 + angle * 0.03) * 10;
