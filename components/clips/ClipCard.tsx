...
export function ClipCard({ clip, view = 'grid' }: ClipCardProps) {
  ...
  return (
    <Link href={`/clips/${clip.id}`}>
      <Card className="hover:border-primary/50 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group h-full">
        ...
        <CardContent className="space-y-4">
          {/* Content */}
          <p className="text-foreground whitespace-pre-wrap break-words leading-relaxed line-clamp-4">
            {clip.content}
          </p>

          {/* Summary */}
          {clip.summary && (
            <p className="text-sm text-muted-foreground italic leading-relaxed border-l-2 border-border pl-3 line-clamp-2">
              {clip.summary}
            </p>
          )}
